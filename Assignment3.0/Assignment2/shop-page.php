<?php
    ...
    $count_query = mysqli_query($conn, "SELECT COUNT(*) as total FROM figures");
    $count_result = mysqli_fetch_assoc($count_query);
    $total_rows = $count_result['total'];

    // Calculate total number of pages
    $total_pages = ceil($total_rows / $items_per_page);
    // Perform SQL query to fetch items for the current page
    $result = mysqli_query($conn, "SELECT * FROM figures LIMIT $offset, $items_per_page");
...
<head>
        ...
        <style>
            ...
            body > section > div:nth-child(7) > nav > ul > li.page-item.active > a{
                outline: none;
                background-color: red;
            }

            body > section > div:nth-child(7) > nav > ul > li.page-item:hover{
                outline: none;
                background-color: yellow;
                color: black;
            }

            body > section > div:nth-child(7) > nav > ul > li:nth-child(n) > a:hover {
                outline: none;
                background-color: yellow;
                color: black;
            }
    ...
    <!-- Keep only one jQuery script tag -->
